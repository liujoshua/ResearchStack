...
public class SageAppPrefs
{
    ...
    // Statics
    //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    private static SageAppPrefs instance;
    ...
    SageAppPrefs(Context context)
    {
        prefs = PreferenceManager.getDefaultSharedPreferences(context);
    }

    public static synchronized SageAppPrefs getInstance(Context context)
    {
        if(instance == null)
        {
            instance = new SageAppPrefs(context);
        }
        return instance;
    }
